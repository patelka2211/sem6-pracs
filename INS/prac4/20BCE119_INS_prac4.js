...
function generate_subkeys(key) {
    ...
    for (let index = 0; index < no_of_rounds; index++) {
        ...
        [left_half, right_half].forEach((half) => {
            half = get_binary(circular_left_shift(parseInt(half, 2), key_shift_values[index], Math.floor(key_size / 2)), Math.floor(key_size / 2));
        });
...
function encrypt(plaintext, keys) {
    let permuted_plaintext = [], ciphertext = [], k = 0, i;
    ...
    keys.forEach((key) => {
        ...
        permuted_plaintext.forEach((block) => {
            let left_half = block.substring(0, 4), right_half = block.substring(4), new_left_half = right_half, new_right_half, temp;
            right_half = get_permuted_value(right_half, plaintext_expansion_permutation);
            temp = xor_operation(parseInt(right_half, 2), parseInt(key, 2));
            right_half = get_binary(temp, block.length);
            right_half =
                perform_substitution(right_half.substring(0, 4), substitution_box_0) +
                    perform_substitution(right_half.substring(4), substitution_box_1);
            right_half = get_permuted_value(right_half, right_half_permutation_box);
            temp = xor_operation(parseInt(right_half, 2), parseInt(left_half, 2));
...
function decrypt(ciphertext, keys) {
    let ciphertext_blocks = [], i = 0, k = 0, plaintext = [];
    while (i < ciphertext.length) {
        ciphertext_blocks.push(get_permuted_value(ciphertext.substring(i, i + plaintext_block_size), plaintext_initial_permutation));
        i += 8;
    }
    keys.forEach((key) => {
        i = 0;
    ...
    ciphertext_blocks.forEach((block) => {
        plaintext.push(String.fromCharCode(parseInt(get_permuted_value(block, inverse_initial_permutation), 2)));
    });
    return plaintext;
}
