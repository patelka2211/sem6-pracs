...
export const encryption = (pt, depth) => {
    pt = cleanPlaintext(pt);
    if (pt.length === 1)
        return pt;
    if (depth === 1 || !(pt.length / depth > 1)) {
        throw Error(`Depth must be in range of [2, ${pt.length - 1}] in this case.`);
    }
    let row = 1, rowStep = 1;
    let matrix = {};
...
export const decryption = (ct, depth) => {
    if (depth === 1 || !(ct.length / depth > 1)) {
        throw Error(`Depth must be in range of [2, ${ct.length - 1}] in this case.`);
    }
    let couter = 0, matrix = {};
